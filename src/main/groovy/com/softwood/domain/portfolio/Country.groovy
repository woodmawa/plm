...
class Country {
    ...
    static countries = new ConcurrentLinkedQueue()
    ...
    //private constructor to help build
    private Country (cname, code2, code3, number) {
        name = cname
        iso2code = code2
        iso3code = code3
    ...
    String toString () {
        "Country ($name, iso2:$iso2code, iso3:$iso3code)"
    }

    static Collection findByNameMatch (partialMatchStr) {
        countries.findAll {it.name.contains (partialMatchStr)}.toList()
    }

    static Country findByIso2 (iso2) {
        countries.find {it.iso2code == iso2}
    }
    static Collection getCountryList () {
        countries.collect {it.name}
    }

    //static initialser to build master list of countries
    static {
        def countryList = [["Afghanistan", 	"AF",	"AFG", "004"],
 ...
 /*                      Algeria	DZ	DZA	012
        ...
        countryList.each {countries << new Country (it[0], it[1], it[2], it[3])}
    }
}
